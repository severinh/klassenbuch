...
//echo "Parsing string - internal way (".$data.")\n";
	...
	function jsonrpc_parse_req($data, $return_phpvals=false, $use_extension=false, $src_encoding='')
	{
		...
		if ($ok)
		{
			...
			if (!is_array($GLOBALS['_xh']['value']) || !array_key_exists('method', $GLOBALS['_xh']['value'])
				|| !array_key_exists('params', $GLOBALS['_xh']['value'])) // Änderung:  || !array_key_exists('id', $GLOBALS['_xh']['value']) entfernt
			{
			...
			else
			{
				...
				$GLOBALS['_xh']['id'] = 32; // Änderung: $GLOBALS['_xh']['value']['id']
				if (!$return_phpvals)
				{
	...
	* @access public
	*/
	function &php_jsonrpc_encode($php_val, $options=Array())
	{
	...
	function serialize_jsonrpcresp($resp, $id=null, $charset_encoding='')
	{
		...
		else
		{
			if(!is_object($resp->val) || !is_a($resp->val, 'xmlrpcval'))
			{
				...
				{
					$result .= "\"result\":" . $resp->val . ",\"error\":null"; // Änderung: "\"error\": null, \"result\": " . $resp->val;
				}
				else
				{
			...
			{
				$result .= "\"result\":" . serialize_jsonrpcval($resp->val, $charset_encoding) . ",\"error\":null";
				// Änderung: $result .= "\"error\": null, \"result\": " . serialize_jsonrpcval($resp->val, $charset_encoding);
			}
		}
		$result .= "}"; // Änderung: $result .= "\n}";
		return $result;
